...
const controlClass =
  'w-full rounded-[10px] border border-field bg-white px-4 font-sans text-[16px] text-ink focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-accent aria-invalid:border-accent-ink';
...
function Field({ name, label, error, children }: FieldProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5 md:gap-2">
      <label htmlFor={id} className="text-sm leading-none font-semibold text-ink">
        {label}
      </label>
      {children({
        id,
      ...
      {error && (
        <p id={errorId} className="text-sm leading-snug text-accent-ink">
          {error}
        </p>
      )}
    </div>
...
export function InquiryForm() {
  ...
  return (
    <form
      ...
      className="card flex flex-col gap-[18px] p-[26px] xl:gap-[22px] xl:p-9"
    >
      <h2 id="inquiry-title" className="type-h3 text-[26px] xl:text-[34px]">
        {copy.title}
      </h2>
      ...
      <div role="status" className="contents">
        {state.status === 'success' && !isPending && (
          <p className="type-body font-semibold text-ink">{copy.success}</p>
        )}
      </div>
      {state.status === 'error' && !isPending && (
        <p role="alert" className="type-body text-accent-ink">
          {state.message}
        </p>
      )}
    </form>
