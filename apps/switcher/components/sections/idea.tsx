...
export function Idea() {
  return (
    <section id="idea" aria-labelledby="idea-title" className="section-pad border-t border-line">
      <div className="container-lab">
        <SectionHeading index={1} eyebrow="The idea" title={idea.heading} titleId="idea-title" />

        <RevealGroup as="ol" stagger={0.12} className="mt-12 grid gap-10 sm:mt-14 md:grid-cols-3 md:gap-8">
        ...
        <RevealGroup as="dl" stagger={0.12} className="mt-14 grid grid-cols-3 border-y border-line sm:mt-16">
          {idea.stats.map((stat, index) => (
            <RevealItem
              key={stat.label}
              className={`flex flex-col-reverse gap-2 py-6 sm:py-8 ${index > 0 ? 'border-l border-line pl-4 sm:pl-8' : 'pr-4'}`}
            >
