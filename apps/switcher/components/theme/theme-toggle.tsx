...
// Copied from apps/motion/components/theme/theme-toggle.tsx (the apps share
// no code). Change: the transition module (with its WebGL shader) is loaded on
// first use instead of with the page, warmed up on hover or focus.
...
let transitionModule: Promise<typeof import('./theme-transition')> | null = null;
const loadTransition = () =>
  (transitionModule ??= import('./theme-transition').catch((error) => {
    // Let a later click retry the download.
    transitionModule = null;
    throw error;
  }));
...
/**
 * Navbar button that switches between the dark and light themes with the
...
export function ThemeToggle({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      aria-pressed={dark}
      onPointerEnter={loadTransition}
      onFocus={loadTransition}
      onClick={(event) => {
        const button = event.currentTarget;
        primeDropSound();
        loadTransition()
          .then(({ toggleTheme }) => toggleTheme(button))
          // Chunk failed to load: still switch, without the transition.
          .catch(() => setTheme(getTheme() === 'dark' ? 'light' : 'dark'));
      }}
      className={cn(
