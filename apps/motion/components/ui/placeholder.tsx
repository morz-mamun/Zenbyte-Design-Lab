...
/**
 * An image position. Renders the dark striped placeholder until the
...
export function Placeholder({ slot, className, sizes = '100vw', priority }: PlaceholderProps) {
    ...
    return (
      <div className={cn('relative overflow-hidden', className)}>
        <Image
          ...
          sizes={sizes}
          priority={priority}
          // SVG mockups are already resolution-independent; skip the optimizer.
  ...
  return (
    <div
      {...a11y}
      className={cn(
        'stripes-dark type-mono flex items-center justify-center border border-line-soft p-2.5 text-center text-muted md:p-3',
        className,
      )}
    >
      {slot.caption}
...
export function Avatar({ initials, image, className }: AvatarProps) {
  ...
  return (
    <span
      aria-hidden="true"
      className={cn(
        'stripes-dark type-mono flex shrink-0 items-center justify-center rounded-full border border-line text-fg-2',
        className,
      )}
    >
      {initials}
